...
- Embed MiSans so the page renders identically without the font installed: `scripts/batch_generate.py` subsets the `Normal`/`Bold`/`Heavy` weights from `assets/fonts/misans-src/MiSans-*.ttf` to the page's visible characters and injects them as woff2 data-URI `@font-face` rules (registered under both `MiSans` at the numeric weight and the `MiSans-{Normal,Bold,Heavy}` aliases) directly after the canonical `<style>` tag. Requires `pip install fonttools brotli`; when the source TTFs or fontTools are unavailable the generator silently falls back to plain font-name references. Do not hand-strip these rules from generated output — see `assets/fonts/README.md`.
