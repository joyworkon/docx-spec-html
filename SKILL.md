...
- Use `.metric-emphasis` for standalone effect/conversion metric lines — in card headers AND inside module bodies — including `效果数据（样本）：XX率+X%`, `效果数据：…2.66%`, and `10SKU（日期）：XX率+X PP`; `%`/`％`/`PP` all qualify. A short standalone conversion line that ends with `↑`/`↓` instead of a numeric value (e.g. `订单转化率↑`, `提升加购率↑，降低退货率↓`) is the same kind of emphasis statement and also uses the green `.metric-emphasis` box — never a plain paragraph or a grey list item. Use `.ba-compare` for before/after comparisons and `.spec-table` for three-column specifications. Always wrap `.ba-compare` in a `.text-block` white container, even with no preceding label: the `.ba-before` header's light-grey fill reads as "no background" when the compare sits directly on the grey panel.
