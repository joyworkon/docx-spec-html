...
8. In the 主图视频 module, replace click-to-watch signals with one pink `点击播放` card; do not retain raw links. Render other DOCX hyperlinks （操作手册 / 平台规则 …) as their anchor text plus one red `.link-btn` `点击查看` pill; never expose raw URLs as text.
