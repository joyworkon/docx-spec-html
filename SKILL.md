...
6. For any `首张主图模块化布局图`, inspect the source image and redraw its exact words, positions, proportions, and blocks with `.module-layout`; never invent a standard template. Match the source image's overall aspect ratio with an inline `aspect-ratio` on the container.
