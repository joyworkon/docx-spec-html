...
- **层级以 Word 列表级 `ilvl` 为准（`paragraph_list_level`）**：段落是否带项目符号/编号级别（`w:numPr/w:ilvl`）是源文件自带的层级信号——`ilvl=None`＝标题/顶层（红方块），`ilvl=0`＝一级子项（灰方块 `.source-list`），`ilvl≥1`＝更深一级（`.source-list li.deep`：再右缩进 + 空心灰方块）。两个补充线索：手敲的 `①②③…`（`CIRCLED_RE`）按灰色子项处理；以冒号结尾的标签（含 `卖点示例组合：（无优先级）` 这种带尾注的，`is_label` 会先剥尾注 `（…）` 再判冒号）仍是红方块标题。带句中冒号但非列表项（`ilvl=None`）的「前缀：内容」（如 `总结：`/`短标题结构规范：`）才作红方块标题；同样写法但 `ilvl=0`（如 `字数要求：`/`型号：`）作灰方块子项。**不要用纯文本长度或冒号位置硬猜层级。**
- **图片标题用图上/原文标题，绝不补「示例图：」**：`grouped_text_block` 不再自动注入「示例图：」。图片用紧邻其上的源文标题：真实标题（`搜索列表页展示`/`参数楼层商详页展示`/`商品参数展示`，由 `looks_like_image_title` 抬升为红方块 `.label-line`）；带冒号的 `示例图：`/`示例：` 仍是红方块标题；纯 `示例`/`示例图`（无冒号）整行**丢弃**（`is_generic_example_label`），不渲染。连续「标题→图→标题→图」拆成各自带标题的图组。
- **首张主图模块化布局图重绘（`module_layout`）**：黄底红框、竖向比例；品牌 LOGO 左上、特性模块左列、产品展示右列、活动腰带底部——从捕获的 `主图首张` 模块名**通用**生成（任何品类都可），不再依赖具体关键词。仍是无视觉时的回退；有识图能力的 agent 应读图上真实文字与比例 1:1 重画。
