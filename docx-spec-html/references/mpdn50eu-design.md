...
- **标签 + 示例图合并同一白底**：当 `1.首张主图模块化布局图：`、`示例：`、`第N屏：…` 等标签后紧跟示例图时，标签（`.label-line` 红方块 + 荧光条）、灰色方块说明 `.caption-line`（文案固定「示例图：」，**只用灰方块、不加红方块和荧光条**）与图片须包在**同一个** `.text-block` 白底里。
...
- **转化率绿色强调条**：当一/二级标题后紧跟独立的 `XX率+X%`（如 `搜索点击率+2.5%`）时，单独一行用 `.metric-emphasis`：左右撑满、绿色描边 + 淡绿底，其中 `X%`（`.metric-value`）放大约两倍。仅匹配独立成行的指标；若指标嵌在更长标签内（如 `2.优化前后图 商详转化率+2%`）则保留在标签中、不单独成条。
