...
- **素材图类型表用 `.material-table`（「示例」表头 colspan=2 跨两列，强约束）**：`4、丰富素材图类型` 那张表按 `素材图类型 ｜ 内容要求 ｜ 示例` 三段排；但「示例」是**一个表头单元格 `<th colspan="2">` 横跨两列**，下方每行（白底图／卖点图／场景图／透明图）在这两列里**各放一张示例图**（共两张并排）。列宽 `mt-c1:16% / mt-c2:34% / mt-c3:25% / mt-c4:25%`；`素材图类型`列居中加粗、`内容要求`列左对齐、示例两列图片 `width:100%` 等宽。不得拆成双列卡片或打乱行列对应。
