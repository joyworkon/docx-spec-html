...
- **多列对比矩阵（优化方向表）用 `.compare-matrix`，第一列按分组 rowspan 合并（强约束）**：当表格是「优化内容 ｜ 案例 ｜ 优化前 ｜ 优化后」这类**四列对比矩阵**（PDF 里「（2）首图内容优化方向」那张）时，**绝不**把它拍平成一行一句的普通表，必须还原成分组矩阵：
  - **第一列「优化内容」是分组名**（如 `突出卖点强调效果`／`同色背景统一质感`／`场景图营造空间感`），**同一组的多行在第一列合并成一个单元格**（HTML `<td rowspan>`；若用 CSS Grid 则该块 `grid-row: span N` 并垂直居中），文字**水平＋垂直居中**、加粗。
  - **每组固定三行**：`案例1`、`案例2`、`优化总结`；`优化总结`行的「优化前」列写问题（如 `页面信息元素多、核心卖点不突出`）、「优化后」列写做法（如 `摒弃冗余数字、量化放大、字体差异化、色块强调保湿提亮`）。
  - 表头四列用 `.spec-table` 同源的表头样式（居中、加粗）；数据单元格一律**水平＋垂直居中**，`优化前/优化后`列若是图片走 `width:100%` 等宽。整张表与其标题（`（2）首图内容优化方向` 抬升为红方块 `.label-line`）同处一个白底卡。
  - **图片单元格必须铺满、与表格边缘上下左右对齐**：放图的 `<td>` 加 `.cm-img`，该 td `padding:0`、内部 `.image-holder` `margin:0;width:100%;min-height:0`、`.doc-image` `width:100%;height:auto;display:block`。**禁止用 `object-fit:contain`+`max-height`**——那会让图片四周留一圈内边距缩小一圈，且 html2canvas 不渲染 object-fit。
- **卖点选词方向表用 `.attr-table`（灰底／红底成对交替，强约束；归属 05 通用卖点，非属性）**：`5、通用卖点` 模块里「卖点选词优先级 / 卖点选词方向」那张表——`适用肤质/香型 ｜ 功效/成分 ｜ 适用人群/净含量`——**属于 05 通用卖点，绝不能当成 08 属性模块的内容**（Word 里这段容易被误挂到属性节；PDF 186-198 明确它在通用卖点下）。表头单元格按成对交替配色——第一组（`适用肤质`／`香型`）与第三组（`适用人群`／`净含量`）用**灰底 `#f2f2f2`、深灰字 `#333`**（`.at-grey`）；中间组（`功效`／`成分`）用**品牌红底 `#FF2B22`、白字**（`.at-red`）；表头加粗居中，数据单元格**水平＋垂直居中**。配色是「灰-红-灰」交替，不是全红或全灰。**这是纯文字表，不含图**；若 Word 原块混进了对比图（`ba-compare` 两列带图），一律清掉图、只保留文字网格。
- **素材图类型表用 `.material-table`（「示例」表头 colspan=2 跨两列，强约束）**：`4、丰富素材图类型` 那张表按 `素材图类型 ｜ 内容要求 ｜ 示例` 三段排；但「示例」是**一个表头单元格 `<th colspan="2">` 横跨两列**，下方每行（白底图／卖点图／场景图／透明图）在这两列里**各放一张示例图**（共两张并排）。列宽 `mt-c1:16% / mt-c2:34% / mt-c3:25% / mt-c4:25%`；`素材图类型`列居中加粗、`内容要求`列左对齐、示例两列图片 `width:100%` 等宽。不得拆成双列卡片或打乱行列对应。**放图的 `<td>` 加 `.mt-eg`，该 td `padding:0`、内部 `.image-holder` `margin:0;width:100%;min-height:0`、`.doc-image` `width:100%;height:auto;display:block`，使图片铺满单元格、与表格边缘上下左右对齐；禁止 `object-fit:contain`+`max-height`（会让图小一圈，且 html2canvas 不渲染 object-fit）。**
- **品质标签示例图用 `.tag-example-table` 翻倍宽（强约束）**：`7、品质标签` 里那张「品质标签示例」表因通用 `.doc-table .doc-image { max-height:300px }` 会被压得很小；给该表加 `.tag-example-table` 类，图片走 `width:100%; max-height:600px`，宽度翻倍看清。「建议以国妆认证为主」这类结论文字放在该品质标签白卡内（`plain-block`）。
- **属性表交替配色表头用 `.attr-table`（灰底／红底成对交替，强约束）**：`8、属性` 模块那张「适用肤质/香型 ｜ 功效/成分 ｜ 适用人群/净含量」的属性表，**表头单元格按成对交替配色**——第一组（`适用肤质`／`香型`）与第三组（`适用人群`／`净含量`）用**灰底 `#f2f2f2`、深灰字 `#333`**（`.attr-head-gray`）；中间组（`功效`／`成分`）用**品牌红底 `#FF2B22`、白字**（`.attr-head-red`）；表头加粗居中，数据单元格常规字重、**水平＋垂直居中**。配色是「灰-红-灰」交替，不是全红或全灰。
...
- **同级并列标题的样式必须一致（`label-text` vs `label-plain` 不能混用）**：同一分组下形如 `（1）…`/`（2）…`/`（3）…`、`示例一`/`示例二` 这类并列的子标题，是同一层级的兄弟节点，必须用**同一种** label 样式——要么都 `.label-text`（红方块＋粉条），要么都 `.label-plain`（仅红方块）。**坑（已犯过）**：`（1）首图模块化`、`（2）…` 用了带粉条的 `.label-text`，而 `（3）主图前五张示例` 因无冒号被判成 `.label-plain`（漏了粉条），导致同级三条标题一条秃、两条带条。冒号判定只决定「粉条盖到哪」，**不应**让同级兄弟里某一条独自掉档；重建时先把并列标题识别成一组，再统一取其中「最高规格」的样式套到全组。另：这类居中示意图块（如 `.module-layout`）在其标题下应水平居中（`margin: … auto …`），不要左贴。
