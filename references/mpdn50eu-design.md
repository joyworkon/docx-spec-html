...
- **「首张主图模块化布局图」重画为 `.module-layout`**：当出现 `…首张主图模块化布局图：` 标签且其后有示意图时，**不直接嵌入带水印的原图**，而是重画为彩色色块 `.module-layout`（**淡黄底 `#fff8e1` + 红框 `2px #ff2b22`**，`max-width:620px` 居中以**保持原图的竖向比例**；品牌 LOGO 左上、主要功能卖点顶部橙色横幅、其他功能黄色胶囊、主品居中、赠品/物流质保两下角、材质/营销卖点底部一行）。
  - **首选（模型视觉重绘）**：安装本 skill 的 agent 应调用自身**多模态识图能力或识图类 MCP**读取该示意图上的**真实文字与各模块的真实比例/位置**，1:1 忠实重画进 `.module-layout`（例如把模块名写成图上的原文「功能功效主要卖点」「多功能（挪车号码牌/手电筒/割刀）」「主品（场景化，1-2个）」「物流信息（自营/京东物流/明日内送达）/质保时间」「主要营销卖点（安全性）」等），生成后替换掉脚本的占位重画。
...
- **首张主图模块化布局图重绘（`module_layout`）＝按原图布局结构 + 每块不同色**：黄底红框（固定）；内部按京东主图标准结构布局——顶行 `店铺名称`(蓝) ｜ `品牌LOGO`(橙)，中部 侧栏卖点(绿) ｜ `商品主图`(紫，大)，底部 质保/售后腰带(粉)；除底色黄、边框红固定外，每块用一种**明显区分**的颜色。生成器从捕获的 `主图首张` 模块名通用映射进这些槽位（无视觉时的回退）。**有识图能力的 agent 必须读“文档里那张布局图”上的真实文字与比例，1:1 还原其布局结构后再上色**（颜色自配、区分明显），不要照搬槽位默认词。
- **主图视频示范（`video_demo_box`）**：`主图视频` 卡里出现 `主图视频示范`/`视频播放按钮`/`播放按钮` 时，渲染淡粉底、居中的「`主图视频示范`（深灰粗体）+ 圆形镂空三角播放图标」横条；校验器视其为有意改写、不计缺字。
...
- **头图干净背景**：Hero 背景＝纯品牌红渐变 `linear-gradient(102deg,#ff3026,#ff1e16)`；去掉网格纹理（`.hero::before`）、圆环（`.rings/.ring`）与高光点，markup 里也不再输出 `.rings`。
