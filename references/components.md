...
- For any source table with more than five effective columns (count after expanding `colspan`/merged cells), transpose the semantic matrix before choosing tracks: the source top headers become the left-side `<th scope="row">` labels, the source left-side row headers become the top `<th scope="col">` labels, and each body value/media cell moves to the transposed intersection. Preserve merged cells and page continuations as one logical matrix; never rotate only the labels or let a continuation revert to the original orientation. If the source has no explicit row-header column, use the strongest meaningful value from each source row as its transposed column label and keep every remaining value/media cell mapped to its field.
- Use `.ba-compare` for two-column 优化前/优化后 material; grey header before, red header after. Always wrap `.ba-compare` in a `.text-block` white container, even when no label precedes it — the `.ba-before` header's light-grey fill is only visible against the white card, never directly on the grey panel. Media cards stretch to the column height (backgrounds stay level; images keep source proportion and stay centred). When the source gives 优化前/优化后 a shared header spanning several image columns (colspan), keep ONE column per header and place its images side by side in `.ba-media-row`; never repeat the header per image. When the body has several pure-image rows (stacked pairs), use the `.ba-compare.is-row-grid` variant: one header row, then each source row's images share one grid row so every pair renders at the same height.
...
- A `.ba-compare` body text cell is a rounded-table body cell, and it is **mandatory**: the copy under a 优化前/优化后 head goes in `.ba-col .ba-text`, which carries the light-grey `#f7f7f7` fill (`--table-body-bg`), the 10px corner radius, the 18px/24px inset, and `flex: 1 0 auto` so both columns end at the same height; text longer than 10 visible characters justifies like any other body cell. A bare `<p>` directly under `.ba-col` — a grey head floating above unframed text with no cell background — is the exact defect `dom_contracts.compare_and_play_checks.ba_compare_text_uses_table_body_cells` rejects. Legal direct children of `.ba-col` are `.ba-head`, `.ba-text`, `.image-holder`, `.ba-media-row`, `.doc-table-wrap`, `.video-demo`, and `.metric-emphasis`.
- A `点击播放` card (`.video-demo`) is table geometry, not a free-floating badge. Inside a table or comparison cell it fills its host (`width: 100%; height: 100%; min-height: 100%; align-self: stretch`) and supplies the canonical 12px `--table-media-padding` itself, while the host cell zeroes its own padding via `table-media-cell` / `video-cell` / `video-case-media`. Two explicit helpers exist: `.video-demo.match-head` matches a header band's `--video-header-height` (72px) and `.video-demo.match-cell` stretches with a body row. A play card that is visibly shorter, taller, or narrower than the adjacent table column fails `dom_contracts.compare_and_play_checks.play_card_matches_neighbour_cell_box`.
